...
const Calendar = () => {
                ...
                if (cellValue && eventName) {
                    ...
                    try {
                        if (typeof cellValue === 'string' && cellValue.includes('/')) {
                            ...
                            if (parts.length === 3) {
                                // Handle DD/MM/YYYY or MM/DD/YYYY based on locale preference or assume DD/MM/YYYY
  ...
  const isEventOnDate = (event, targetDate) => {
    if (!event.date || !(event.date instanceof Date) || isNaN(event.date)) return false;
  ...
  const getGroupedMonthEvents = () => {
    ...
    const currentMonthEvents = events.filter(event => {
        ...
        if (effectiveDate.getMonth() !== currentDate.getMonth() || 
  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 0; i < startDay; i++) {
        days.push(<div key={`empty-${i}`} className="bg-gray-50/20 border-r border-b border-gray-100"></div>);
    }
            ...
            >
                ...
                <div className="flex-1 overflow-hidden mt-1 space-y-0.5">
                    ...
                    {uniqueEventNames.length > 3 && (
                        <div className="text-[10px] text-gray-400 pl-1">+{uniqueEventNames.length - 3} more</div>
                    )}
                </div>
            </div>
        );
    ...
    <div className="h-[calc(100vh-4rem)] flex flex-col md:flex-row bg-gray-50 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col h-full overflow-hidden p-4">
            ...
            <div className="flex items-center justify-between mb-4 bg-white p-3 rounded-xl border border-gray-200 shadow-sm flex-shrink-0">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={prevMonth} className="p-1.5 hover:bg-gray-100 rounded-lg border border-gray-200"><ChevronLeft className="w-4 h-4" /></button>
                    <button onClick={() => setCurrentDate(new Date())} className="px-3 py-1.5 border border-gray-200 rounded-lg text-sm font-medium hover:bg-gray-50">Today</button>
                    <button onClick={nextMonth} className="p-1.5 hover:bg-gray-100 rounded-lg border border-gray-200"><ChevronRight className="w-4 h-4" /></button>
                </div>
            </div>
            ...
            <div className="flex-1 bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col overflow-hidden">
                ...
                {loading ? (
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-red-500 p-4 text-center">
                        ...
                        <button onClick={fetchCalendarData} className="mt-4 px-4 py-2 bg-white border border-red-200 rounded-lg text-sm hover:bg-red-50">Retry</button>
                    </div>
                ) : (
        ...
        <div className="w-full md:w-80 bg-white border-l border-gray-200 flex flex-col h-full shadow-lg z-10">
            <div className="p-4 border-b border-gray-100 bg-gray-50/50 flex flex-col gap-1 flex-shrink-0">
                ...
                <p className="text-xs text-gray-500">Summary for {months[currentDate.getMonth()]}</p>
            </div>
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500">{group.events.length} Parties celebrating</p>
                                </div>
                            </div>
                    ...
                    >
                        <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-indigo-50/50">
                            <h2 className="font-bold text-gray-900 flex items-center gap-2">
                                <span className="text-indigo-600">{selectedDate.date.getDate()} {months[selectedDate.date.getMonth()]}</span>
                                Details
                            </h2>
                        ...
                        <div className="p-4 overflow-y-auto space-y-4">
                            {selectedDate.events.map((event, idx) => (
                                <div key={idx} className="bg-white border border-gray-100 rounded-xl p-3 shadow-sm">
                                    ...
                                    <p className="text-sm text-indigo-600 font-medium mt-1">{event.eventName}</p>
                                </div>
                            ))}
                        </div>
                    </motion.div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    ...
                    >
                        <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-emerald-50/50">
                            ...
                            <button onClick={() => setSelectedFestival(null)} className="p-1 hover:bg-gray-200 rounded-full"><X className="w-5 h-5 text-gray-500" /></button>
                        </div>
                         <div className="p-2 border-b border-gray-50 bg-gray-50/30">
                            <p className="text-xs text-center text-gray-500">
                                Showing all {selectedFestival.events.length} parties celebrating {selectedFestival.name} this month
                            </p>
                        </div>
                        <div className="p-4 overflow-y-auto space-y-2">
                            {selectedFestival.events.sort((a,b) => a.date - b.date).map((event, idx) => (
                                <div key={idx} className="flex items-center justify-between bg-white border border-gray-100 rounded-xl p-3 shadow-sm hover:bg-gray-50 transition-colors">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500 mt-0.5">{event.date.getDate()} {months[event.date.getMonth()]}</p>
                                    </div>
